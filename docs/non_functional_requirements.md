...
*   **Authorization:** Roles are assigned based on the class the user belongs to within the school system, and their status as either teacher, student or parent. Changes in roles must be propagated to the individual components within one day.
...
*   **Audit Logging:** The system must provide a broad audit trace. The following actions must be logged with user attribution:
    ...
    *   Manipulation of data by techniccal staff
*   **Data Retention:**
...
*   **Browser Support:** The system must function correctly on all modern browsers.
    *   **Priority:** Support for the Firefox browser on Linux is critical, as this is the standard configuration for school computers.
*   **Mobile Access:** A phone-appropriate user interface must be accessible via the mobile browser. Development of a dedicated native mobile application is not required.
...
*   **Response Times:**
    *   **Standard Operation:** The majority of tasks and navigation should take no longer than **1 second**.
    *   **High Load (General):** Navigation may take up to **10 seconds**.
...
*   **Data Sovereignty:** All data must be processed and stored within the **European Union (EU)**.
*   **Vendor Neutrality:**
    *   Software providers must not utilize proprietary customizations to the program code.
